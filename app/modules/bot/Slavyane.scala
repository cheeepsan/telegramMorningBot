...
class Slavyane(val botToken: String, val db: Db) extends TelegramBot
  ...
  def happyNewYear1() = {
    val file: InputFile = InputFile(new File("files/slav_new_year.jpg").toPath)
    this.sendFile(file)
  }

  def happyNewYear2() = {
    val file: InputFile = InputFile(new File("files/slav_new_year2.jpg").toPath)
    this.sendFile(file)
  }

  def sendFile(file: InputFile): Unit = {
